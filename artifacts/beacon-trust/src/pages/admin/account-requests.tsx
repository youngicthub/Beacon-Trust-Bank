...
export default function AdminAccountRequests() {
  ...
  const fetchAccounts = async () => {
    setIsLoading(true);
    // Map filter to Supabase enum values
    ...
    const { data, error } = await supabase
      .from('accounts')
      .select('id, account_number, type, currency, balance, status, created_at, users!inner(id, email, first_name, last_name)')
      .eq('status', statusMap[filter] as 'active' | 'frozen' | 'closed' | 'pending')
                        ...
                        {filter === 'pending' && (
                          <>
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => handleAction(acc.id, 'reject')}
