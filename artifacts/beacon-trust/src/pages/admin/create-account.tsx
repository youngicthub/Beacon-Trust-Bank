...
import { UserPlus, CheckCircle2, Eye, EyeOff } from '@/lib/icons';
...
const createAccountSchema = z.object({
  ...
  accountType: z.enum(['savings', 'current', 'business', 'investment']).optional(),
});
...
export default function AdminCreateAccount() {
  ...
  const form = useForm<CreateAccountForm>({
    ...
    defaultValues: { role: 'customer', initialBalance: 0, accountType: 'savings' },
  });

  const onSubmit = async (data: CreateAccountForm) => {
    setIsSubmitting(true);
    try {
      await apiFetch('/api/admin/create-customer', {
        method: 'POST',
        body: JSON.stringify({
    ...
    <DashboardLayout>
      <div className="max-w-2xl mx-auto space-y-6">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          ...
          <p className="text-muted-foreground mt-1">Provision a new user account directly from the admin panel.</p>
        </motion.div>
              ...
              <Card className="border-border/50 shadow-lg">
                <CardHeader className="border-b border-border/50 bg-muted/20">
                  <CardTitle className="flex items-center gap-2">
                    <UserPlus className="h-5 w-5 text-primary" /> Account Provisioning
                  </CardTitle>
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                      <div className="grid grid-cols-2 gap-4">
                        <FormField control={form.control} name="firstName" render={({ field }) => (
                          <FormItem><FormLabel>First Name *</FormLabel><FormControl><Input {...field} className="h-11" placeholder="John" /></FormControl><FormMessage /></FormItem>
                        )} />
                        <FormField control={form.control} name="lastName" render={({ field }) => (
                          <FormItem><FormLabel>Last Name *</FormLabel><FormControl><Input {...field} className="h-11" placeholder="Smith" /></FormControl><FormMessage /></FormItem>
                        )} />
                      </div>

                      <FormField control={form.control} name="email" render={({ field }) => (
                        <FormItem><FormLabel>Email Address *</FormLabel><FormControl><Input {...field} type="email" className="h-11" placeholder="john.smith@example.com" /></FormControl><FormMessage /></FormItem>
                      )} />
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <FormField control={form.control} name="phone" render={({ field }) => (
                          <FormItem><FormLabel>Phone (optional)</FormLabel><FormControl><Input {...field} className="h-11" placeholder="+1 555 000 0000" /></FormControl><FormMessage /></FormItem>
                        )} />
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <FormField control={form.control} name="initialBalance" render={({ field }) => (
                          <FormItem>
                            ...
                            <FormDescription className="text-xs">Starting balance for the new account.</FormDescription>
                            <FormMessage />
                          </FormItem>
                        )} />
                      </div>
