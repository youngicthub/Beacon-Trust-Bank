...
export default function StaffTickets() {
  ...
  const fetchTickets = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select('id, subject, description, status, priority, staff_notes, created_at, updated_at, users!inner(id, first_name, last_name, email)')
      .eq('status', filter)
  ...
  const handleUpdate = async () => {
    ...
    const { error } = await supabase
      .from('support_tickets')
      .update({ status: status as 'open' | 'inProgress' | 'resolved' | 'closed', staff_notes: notes, updated_at: new Date().toISOString() })
                    ...
                    <div className="flex flex-col sm:flex-row justify-between gap-4">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          ...
                          {user && (
                            <Link href={`/staff/customers/${user.id}`} className="text-xs text-primary hover:underline flex items-center gap-1 mt-1">
                              <User className="h-3 w-3" />{user.first_name} {user.last_name} — {user.email}
                            </Link>
                          )}
